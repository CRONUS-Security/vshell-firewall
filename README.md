...
**日志说明：**
- 当配置了 `log_file` 时，日志会**同时**输出到控制台和文件
- 日志格式包含日期、时间和微秒级时间戳
- 日志文件以追加模式打开，不会覆盖旧日志
...
```toml
[global.time_window]
enabled = true           # 是否启用时间窗口过滤
timezone = "UTC"         # 时区设置，如 "UTC", "Asia/Shanghai"
start_time = "00:00"     # 允许连接的开始时间 (HH:MM 格式)
end_time = "11:00"       # 允许连接的结束时间 (HH:MM 格式)
```

**时间窗口说明：**
- 只在配置的时间范围内允许新的 TCP 连接
- 窗口外时，现有连接保持不变，但新连接请求会被丢弃
- 支持跨天时间段（如 23:00 - 02:00）
